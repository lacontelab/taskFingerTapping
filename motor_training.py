...
TCPIP_PORT    = 5000 # Port receiving TCP/IP data
...
LABEL_FILE    = 'lbls_vegg_motor_trn_afniDemo.dat' # label file controlling presentation
...
log_file.write("# time;currBlock;currShow;fbScore;fbScale;l_count;r_count\n")
...
def getState(t):
    ...
    global tcp_listner, dist, curr_slope, curr_offset, dist_array
    ...
    if t > next_TR_time:
      ...
      if TCPIP:
        ...
        if tcp_data != '\000' and tcp_data != "":
          ...
          fbScale = discretizeDist(dist) 
          print t, dist, fbScale

      # get data from serial port 
      elif SERIAL: 
          ####	# note: the current ICE program is sending values
          ####	#	ranging from 1-4 (very negative to small neg),
          ####	#	5 (effictively 0)
          ####	#	6-9 (small positive to large positive)
        if ser.inWaiting() :
          fbScale = ser.read(1)
          ser.flushInput()
      # use values from labelfile 
      else: 
      ...
      log_file.write("%5.3f;%d;%d;%5.3f;%d;%d;%d\n" % (t, currBlock, currShow, fbScore, int(fbScale), l_count, r_count))
...
def keydown(event):
    ...
    if event.key == pygame.locals.K_1 or event.key == pygame.locals.K_2:
            l_count += 1
    elif event.key == pygame.locals.K_3 or event.key == pygame.locals.K_4:
            r_count += 1
